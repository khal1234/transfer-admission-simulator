"""인천대 2024~2026 편입학 모집요강의 학과별 배점을 반영한다.
...
def validate(standard: list[dict], exceptions: list[dict]) -> None:
    by_year = {
        year: sum(
            is_incheon(record) and record["연도"] == year
            for record in standard
        )
        for year in ("2024", "2025", "2026")
    }
    if by_year != {"2024": 53, "2025": 49, "2026": 52}:
        raise RuntimeError(f"인천대 지원 데이터 연도별 건수 불일치: {by_year}")

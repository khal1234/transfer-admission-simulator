...
NAME_ALIASES = {
    ...
    # 24_부산대_성적.pdf 2쪽에 '디자인앤테크놀리저전공' 으로 찍혀 있다.
    # 26_부산대_성적.pdf 2쪽은 '디자인앤테크놀로지전공' 이고 results 도 그쪽을
    # 따랐다. 원본 오타라 이름만 다르고 값은 같다.
    ("부산대학교", "디자인학과\n(디자인앤테크놀리저전공)"):
        "디자인학과(디자인앤테크놀로지전공)",
}

# 원본에는 있으나 이 추출기가 읽지 않는 시트/표에서 온 레코드.
# 값이 틀린 것이 아니라 대조 경로가 닿지 않는 것이라 고아로 세지 않는다.
#
#   부경대 2024 미래융합대학 4개 전공 — 24_부경대_성적.xlsx 는 시트가 둘인데
#   이 추출기는 '일반편입 평균성적 현황' 만 읽는다. 이 넷은 합격자가 없어
#   성적 시트에 행이 없고, '편입학모집 전형별 지원현황' 시트에만 모집-지원
#   인원이 실려 있다. results 의 모집 10/지원 25 등은 그 시트에서 온 값이다.
KNOWN_OTHER_SHEET = {
    ("부경대학교", "2024", normalize_name("평생교육·상담학전공")),
    ("부경대학교", "2024", normalize_name("경찰범죄심리학전공")),
    ("부경대학교", "2024", normalize_name("스마트기계모빌리티전공")),
    ("부경대학교", "2024", normalize_name("스마트전기전자공학전공")),
}
...
def main():
    ...
    for identity, record in sorted(record_ids.items()):
        ...
        if known in KNOWN_UNEXTRACTED or known in KNOWN_OTHER_SHEET:
            continue
        orphans.append(
